...
const Sidebar: React.FC<SidebarProps> = ({ toggleKeyword, selectedKeywords }) => {
    ...
    <aside className="w-[400px] border-r border-gray-800/50 h-[calc(100vh-73px)] sticky top-[73px] backdrop-blur-sm transition-all duration-300 ease-in-out">
      <div className="p-4 border-b border-gray-800/50 backdrop-blur-sm bg-gray-900/50 transition-colors duration-300">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-white transition-opacity duration-300 hover:opacity-80">
            Latest News
          </h2>
        ...
        {/** 2) Filter Buttons from the news */}
        <div className="flex flex-wrap gap-2">
          {filterKeywords.map((filter) => {
            const isSelected = selectedKeywords.includes(filter);
            return (
              <button
                key={filter}
                onClick={() => toggleKeyword(filter)}
                className={`px-3 py-1.5 rounded-full text-sm transition-all duration-300 ease-in-out hover:scale-105 hover:shadow-lg hover:shadow-blue-500/10 transform hover:-translate-y-0.5
                  ${
                    isSelected
